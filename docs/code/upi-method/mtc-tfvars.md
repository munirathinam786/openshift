...
Variable definitions for MTC (Migration Toolkit for Containers) — migrating containerized workloads between OpenShift clusters via the UPI DC Primary cluster.

!!! info "File Location"
    `upi-method/openshiftbaremetal/mtc.tfvars`
...
```hcl
--8<-- "upi-method/openshiftbaremetal/mtc.tfvars"
```
