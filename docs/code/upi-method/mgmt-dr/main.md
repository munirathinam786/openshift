# Management DR (UPI) — main.tf

Main orchestration for the UPI Management DR cluster (`upi-method/mgmt-dr/`).
Deploys OCP on bare metal using UPI with ACM Standby, ACS SecuredCluster only
(Central is in mgmt-dc), Quay Enterprise, ODF, and MetalLB.
...
# --- Day-2: ACS (SecuredCluster ONLY — no Central) ---
module "acs" {
  ...
  central_endpoint       = var.acs_central_endpoint
