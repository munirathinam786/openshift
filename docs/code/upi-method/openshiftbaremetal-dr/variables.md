# UPI DR Secondary — variables.tf

Variable declarations for the UPI DR Secondary workload cluster.
Extends the DC Primary variables with **Submariner agent** and **ODF DR** variables, and uses **non-overlapping CIDRs** for cross-cluster networking.

!!! info "Key Differences from DC Primary"
    - **No BMC fields** in node definitions (UPI — same as DC Primary)
    - **Submariner agent** variables: `submariner_broker_api_url`, `submariner_broker_token`, `submariner_broker_ca`
    - **ODF DR** variables: `odf_dr_mode`, `odf_dr_replication_schedule`, `odf_dr_peer_cluster_name`, S3 vars
    - **CIDRs**: `cluster_network_cidr = 10.132.0.0/14`, `service_network_cidr = 172.31.0.0/16`

## Variable Categories

| Category | Variables | Notes |
|----------|-----------|-------|
| Cluster Identity | `cluster_name`, `base_domain`, `ocp_version` | DR site domain |
| UPI Install | `install_dir`, `rhcos_iso_url`, `boot_method`, `bootstrap_ip/mac`, `ignition_http_port` | Same as DC Primary |
| Networking | `machine_network_cidr`, `cluster_network_cidr`, `service_network_cidr`, VIPs | Non-overlapping with DC |
| Nodes (no BMC) | `master_nodes`, `worker_nodes` | `gpu_worker`, `odf_worker` flags |
| GPU / AI | `ngc_api_key`, `vgpu_driver_*`, `oai_components` | Same as DC Primary |
| Submariner Agent | `submariner_broker_api_url`, `submariner_broker_token`, `submariner_broker_ca`, `submariner_cable_driver` | Agent connects to DC broker |
| ODF DR | `odf_dr_mode`, `odf_dr_replication_schedule`, `odf_dr_peer_cluster_name`, S3 vars | Ceph RBD mirroring |
| LDAP / OAuth | `enable_ldap`, `ldap_url`, `ldap_bind_dn`, `ldap_bind_password`, `ldap_group_role_bindings`, `disable_kubeadmin` | Corporate identity provider |
| GitOps (ArgoCD) | `enable_openshift_gitops`, `gitops_channel`, `argocd_ha_enabled`, `argocd_*_cpu/memory_*`, `argocd_cluster_admin`, `argocd_rbac_*`, `argocd_managed_namespaces`, `argocd_repo_*` | Operator + ArgoCD instance |
| Pipelines (Tekton) | `enable_openshift_pipelines`, `pipelines_channel`, `tekton_profile`, `tekton_api_fields`, `enable_*_cluster_tasks`, `enable_pipelines_as_code`, `pipeline_default_*`, `pipeline_namespaces`, `pipeline_container_*` | Operator + TektonConfig |
...
variable "base_domain" {
  description = "Base DNS domain (e.g. dr.example.com)"
  type        = string
}
...
variable "machine_network_cidr" {
  description = "Machine network CIDR (e.g. 10.143.41.0/24)"
  type        = string
}

variable "cluster_network_cidr" {
  description = "Cluster (pod) network CIDR — non-overlapping with DC Primary"
  type        = string
...
variable "service_network_cidr" {
  description = "Service network CIDR — non-overlapping with DC Primary"
  type        = string
  default     = "172.31.0.0/16"
}

variable "api_vip" {
  description = "Virtual IP for the Kubernetes API (managed by external LB)"
  type        = string
}

variable "ingress_vip" {
  description = "Virtual IP for OpenShift Ingress (managed by external LB)"
  type        = string
}
...
variable "install_dir" {
  description = "Directory on bastion where install-config.yaml, manifests, and ignition configs are generated"
  type        = string
  default     = "/home/kni/ocp-install"
}

# ---- RHCOS Images (UPI requires manual boot) ----
variable "rhcos_iso_url" {
  description = "HTTP URL to the RHCOS live ISO image for PXE/ISO boot"
  type        = string
}

variable "rhcos_rootfs_url" {
  description = "HTTP URL to the RHCOS rootfs image for coreos-installer"
  type        = string
}

variable "install_disk" {
  description = "Target disk device for RHCOS installation (e.g. /dev/sda)"
  type        = string
  default     = "/dev/sda"
}
...
variable "boot_method" {
  description = "How to boot nodes: pxe, iso, or manual (operator boots nodes manually)"
  type        = string
  default     = "pxe"
  validation {
    condition     = contains(["pxe", "iso", "manual"], var.boot_method)
    error_message = "boot_method must be one of: pxe, iso, manual"
  }
}

# ---- Ignition HTTP Server ----
variable "ignition_http_port" {
  description = "Port for the HTTP server that serves ignition configs from bastion"
  type        = number
  default     = 8080
}

# ---- Bootstrap Node (UPI-specific, ephemeral) ----
...
variable "bootstrap_mac" {
  description = "MAC address of the bootstrap node (for PXE)"
  type        = string
}

# ---- Master / Control Plane Nodes ----
variable "master_nodes" {
  description = "List of master/control-plane node definitions (UPI — no BMC)"
  type = list(object({
    name            = string
    ip              = string
    mac_address     = string
...
variable "worker_nodes" {
  description = "List of worker (compute) node definitions (UPI — no BMC)"
  type = list(object({
    name            = string
    ip              = string
    mac_address     = string
...
# ---- HAProxy / External Load Balancer (required for UPI) ----
variable "haproxy_hosts" {
  description = "List of HAProxy LB hosts (required for UPI — external LB for API + Ingress)"
  type = list(object({
    host    = string
    user    = string
    ssh_key = string
...
variable "ocp_channel" {
  description = "OCP update channel for oc-mirror (e.g. stable-4.15)"
  type        = string
  default     = "stable-4.15"
}
...
# ---- Submariner (Agent — connects to DC Primary broker) ----
