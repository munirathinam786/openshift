# Agent Builder Factory — main.tf

Main orchestration file for the Agent Builder Factory platform (`ipi-method/agent-builder/`).
...
!!! info "Deployment Pattern"
    Uses `null_resource` + SSH `remote-exec` + `oc` CLI to deploy Kubernetes manifests to OpenShift — consistent with all other cluster deployments in this project.
