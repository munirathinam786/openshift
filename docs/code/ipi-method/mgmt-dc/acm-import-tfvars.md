# IPI Management DC — acm-import.tfvars

Configuration for importing workload clusters into the ACM Hub on the IPI Management DC cluster.
Used with the [ACM Cluster Import Pipeline](../../../pipeline/terraform-acm-import-pipeline.md).
...
| `acm_managed_clusters` | list(object) | See below | Clusters to import |
| `acm_import_scope` | string | `all-workload` | Pipeline scope override |

## Managed Clusters

| Cluster | API URL | Labels | Add-ons |
|---------|---------|--------|---------|
| `dc-primary` | `https://api.dc-primary.example.com:6443` | `site=dc`, `dr-role=primary` | All enabled |
| `dr-secondary` | `https://api.dr-secondary.example.com:6443` | `site=dr`, `dr-role=secondary` | All enabled |
