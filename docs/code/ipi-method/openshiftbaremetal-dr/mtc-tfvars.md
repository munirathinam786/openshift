...
Variable definitions for MTC (Migration Toolkit for Containers) — migrating containerized workloads between OpenShift clusters via the DR Secondary cluster.

!!! info "File Location"
    `ipi-method/openshiftbaremetal-dr/mtc.tfvars`
...
```hcl
--8<-- "ipi-method/openshiftbaremetal-dr/mtc.tfvars"
```
