# ADO Pipeline — IPI Day 2 Operations

This pipeline runs **Day 2 post-install operations** separately from the Day 1 cluster installation pipeline. Day 2 covers Cluster Logging, OADP Backup & Restore, LDAP/OAuth integration, OpenShift GitOps (Argo CD), and OpenShift Pipelines (Tekton).

!!! info "Day 1 vs Day 2"
    - **Day 1** (`azure-pipelines.yml`) — Cluster install, core operators, networking, storage
    - **Day 2** (`azure-pipelines-day2.yml`) — Logging, backup/restore, identity provider, GitOps (ArgoCD), Pipelines (Tekton)

## Pipeline Flow

![Day-2 Pipeline Flow (IPI)](../diagrams/pipeline/06-day2-pipeline-flow.svg){: .drawio-diagram }

???+ note "Draw.io Source: Day-2 Pipeline Flow (IPI)"
    [:material-download: Download .drawio file](../diagrams/pipeline/06-day2-pipeline-flow.drawio){ .md-button } — Open in [draw.io](https://app.diagrams.net) for interactive editing.
...
| `variableGroup` | string | `ocp-baremetal-day2-secrets` | — | ADO Variable Group name for Day 2 secrets |

## ADO Variable Group — Day 2 Secrets

Create a Variable Group named **`ocp-baremetal-day2-secrets`** in Azure DevOps (**Project Settings → Pipelines → Library → + Variable Group**):
...
|-------|-------------|----------------|-----------|
| `Day2_DC_Primary` | Day 2 — DC Primary | `openshiftbaremetal/` | `dc-only`, `dc-and-dr`, `all-dc`, `all` |
| `Day2_DR_Secondary` | Day 2 — DR Secondary | `openshiftbaremetal-dr/` | `dr-only`, `dc-and-dr`, `all-dr`, `all` |
| `Day2_Mgmt_DC` | Day 2 — Management DC | `mgmt-dc/` | `mgmt-dc-only`, `mgmt-clusters`, `all-dc`, `all` |
| `Day2_Mgmt_DR` | Day 2 — Management DR | `mgmt-dr/` | `mgmt-dr-only`, `mgmt-clusters`, `all-dr`, `all` |

### Stage Dependencies

- **DR Secondary** waits for DC Primary when scope is `dc-and-dr` or `all`
- **Mgmt DR** waits for Mgmt DC when scope is `mgmt-clusters` or `all`
...
See the full pipeline YAML: [azure-pipelines-day2.yml](../code/ipi-method/pipeline/azure-pipelines-day2.md)
